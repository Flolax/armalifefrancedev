...
*/
if (license_driver) exitWith {["Moniteur Auto-Ecole", "Tu as déjà le permis de conduire.", "danger", false] spawn ALF_fnc_doMsg;};
if (life_permis_action) exitWith {["Moniteur Auto-Ecole", "Tu passes déjà ton permis.", "danger", false] spawn ALF_fnc_doMsg;};

_price = 925;

_action = [format["Veux-tu passer ton permis pour %1€. Attention, épreuve sous 3 erreurs éliminatoires.",_price],"Inscription au permis de conduire","OUI","NON"] call BIS_fnc_guiMessage;

if(_action) then {

	if(count(nearestObjects[(getMarkerPos "p_lepalais_start"),["Car","Ship","Air"],5]) > 0) exitWith {["Moniteur Auto-Ecole", "Un vehicule gêne l'auto école.", "danger", false] spawn ALF_fnc_doMsg;};
	...
	//_vehicle = createVehicle ["ALF_308_auto_m", (getMarkerPos "p_lepalais_start"), [], 0, "NONE"];
	_vehicle = createVehicle ["ALF_veyron_black", (getMarkerPos "p_lepalais_start"), [], 0, "NONE"];
	waitUntil {sleep 0.3; !isNil "_vehicle" && {!isNull _vehicle}};
	...
	for "_i" from 0 to 1 step 0 do {
		...
		if (typeOf (vehicle player) isKindOf "ALF_veyron_black") exitWith {_etape1 = true;};
		sleep 1;
	};
	if (_exit) exitWith {hint "Tu as râté ton permis, le moniteur est parti."};
	...
	if (_exit) exitWith {deleteVehicle _vehicle; player allowDamage false; player setposATL _startPos; player allowDamage true; ["Moniteur Auto-Ecole", "Tu as râté ton permis, tu es partie trop loin.", "warning", false] spawn ALF_fnc_doMsg;};
	...
	["G", "15m", "Garez vous sur le parking et arrêtez le véhicule."] spawn ALF_fnc_doGps;
	...
	sleep 2;
	if (_erreur < 5) then {
		["Moniteur Auto-Ecole", "Vous avez votre permis, félicitation !", "success", false] spawn ALF_fnc_doMsg;
		license_driver = true;
		life_permis_action = false;
		[1] spawn SOCK_fnc_updatePartial;
	} else {
