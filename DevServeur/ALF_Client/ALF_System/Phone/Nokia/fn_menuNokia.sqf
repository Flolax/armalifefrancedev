...
switch (_menu) do {
	case 0 : {
		...
		if (life_callprogress) then {
			...
			_hisName = "Inconnu";
			_hisNumber = player getVariable "callNumber";
			_hisAnonyme = player getVariable "callAnonyme";
			if (count life_contacts > 0) then {
				{
					if ((_x select 1) isEqualTo _hisNumber) then {
			...
			if(_hisAnonyme isEqualTo 1) then {_hisName = "Anonyme"; _hisNumber = "**********"};};};
			...
			_menu1raccrocher buttonSetAction "[] spawn ALF_fnc_resetCall;";
		} else {
		if (life_phonering) then {
				...
				_menu1name ctrlSetStructuredText parseText format["<t align='right' shadow='0' color='#000000' font='NokiaCellphoneFC' size='.6'>%1</t>", _hisName];
				_menu1num ctrlSetStructuredText parseText format["<t align='right' shadow='0' color='#000000' font='NokiaCellphoneFC' size='.6'>%1</t>", _hisNumber];
				_menu1decrocher buttonSetAction "[] spawn ALF_fnc_answerCall;";
				_menu1refuser buttonSetAction "[] spawn ALF_fnc_resetCall;";
			} else {
				if(life_trycall) then {
					_menu1incall ctrlShow false;
					_menu1sim ctrlShow false;
					_menu1nosim ctrlShow false;
					_menu1ring ctrlShow false;
					...
					if(life_forfait isEqualTo -1 || life_forfait isEqualTo 0) then {
						_menu1incall ctrlShow false;
						_menu1try ctrlShow false;
						_menu1sim ctrlShow false;
	...
	case 2 : {
		...
		_menu1up ctrlShow false;
		_menu1down ctrlShow false;
		[(player getvariable "myPhoneNumber") select 1,player] remoteExecCall ["HC_fnc_loadSmsNokia",HC_Life];
	...
	case 3 : {
		...
		_menu1num ctrlSetStructuredText parseText format["<t align='right' shadow='0' color='#000000' font='NokiaCellphoneFC' size='.6'>Mon numéro: %1</t>", _myNumber];
	...
	case 4 : {
		...
		if (count life_phone_historique > 0) then {
			{
				...
				if(_anonyme isEqualTo 1) then {_hisName = "Anonyme"; _number = "**********"};
				if (count life_contacts > 0) then {
					{
						if ((_x select 1) isEqualTo _number) then {
							_hisName = _x select 0;
						};
					} forEach life_contacts;
				};
				...
				if (_type isEqualTo 0) then {
					_menurecentlist lbSetPicture [(lbSize _menurecentlist)-1,"\ALF_Client\icons\out.paa"];
				} else {
					_menurecentlist lbSetPicture [(lbSize _menurecentlist)-1,"\ALF_Client\icons\in.paa"];
				};
				_menurecentlist lbSetData [(lbSize _menurecentlist)-1,str(_x)];
			} forEach life_phone_historique;
		} else {
		...
		_menu4supprimer buttonSetAction "life_phone_historique = []; [] call ALF_fnc_menuNokia;";
