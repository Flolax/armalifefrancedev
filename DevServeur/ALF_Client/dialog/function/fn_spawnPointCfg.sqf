...
*/
private["_return","_spawnCfg","_curConfig","_name","_licenses","_level","_levelName","_levelValue","_levelType","_tempConfig","_flag"];

_return = [];

_spawnCfg = missionConfigFile >> "CfgSpawnPoints";
...

    {
      if(!(EQUAL(SEL(_x,0),""))) then {
        ...
        if(_licenseType isEqualTo 0) then {
          if(LICENSE_VALUE(_licenseName)) exitWith {_flag = false};
        } else {
          if!(LICENSE_VALUE(_licenseName)) exitWith {_flag = false};
        };
      };
