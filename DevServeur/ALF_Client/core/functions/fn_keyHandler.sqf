...
switch (_code) do {
	...
	case 19: {
		...
		if(_shift && playerSide isEqualTo west && {!isNull cursorObject} && {cursorObject isKindOf "Man"} && {(isPlayer cursorObject)} && {(side cursorObject in [civilian,independent])} && {alive cursorObject} && {cursorObject distance player < 3.5} && {!(cursorObject GVAR "Escorting")} && {!(cursorObject GVAR "restrained")} && {speed cursorObject < 1}) then {
	...
	case 34: {
		...
		if(_shift && playerSide isEqualTo civilian && !isNull cursorObject && cursorObject isKindOf "Man" && isPlayer cursorObject && alive cursorObject && cursorObject distance player < 4 && speed cursorObject < 1) then {
			if((animationState cursorObject) != "Incapacitated" && (currentWeapon player isEqualTo primaryWeapon player OR currentWeapon player isEqualTo handgunWeapon player) && currentWeapon player != "" && !life_knockout && !(player GVAR ["restrained",false]) && !life_istazed && !life_isknocked) then {
	...
	//Y Player Menu
	case 21: {
		if(!_alt && !_ctrlKey && !_shift && !dialog && !(player GVAR ["restrained",false]) && {!life_action_inUse}) then {
				[] call ALF_fnc_menuY;
		};
		if(!_alt && !_ctrlKey && _shift && !dialog && !(player GVAR ["restrained",false]) && {!life_action_inUse}) then {
				if([(call TFAR_fnc_activeSwRadio),"alf_3310"] call TFAR_fnc_isSameRadio) then { [] call ALF_fnc_menuNokia; };
				if([(call TFAR_fnc_activeSwRadio),"alf_iphone"] call TFAR_fnc_isSameRadio) then {	[] call ALF_fnc_menuiPhone;	};
				if([(call TFAR_fnc_activeSwRadio),"alf_neogend"] call TFAR_fnc_isSameRadio) then {	[] call ALF_fnc_menuSony;	};
		};
		if(_alt && !_ctrlKey && !_shift && !dialog && !(player GVAR ["restrained",false]) && {!life_action_inUse}) then {
				[] call ALF_fnc_menuGendarme;
		};
	};
