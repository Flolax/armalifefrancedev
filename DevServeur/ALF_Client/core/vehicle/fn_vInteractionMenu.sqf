...
if(playerSide isEqualTo west) then {
	...
	if(_curTarget isKindOf "Ship") then {
		...
		if((typeOf (_curTarget) in _dlcVehicles) && !(288520 in getDLCs 1)) then {
			...
			if(alive _curTarget && {count crew _curTarget isEqualTo 0} && {canMove _curTarget}) then { _Btn6 ctrlEnable false;} else {_Btn6 ctrlEnable true;};
		};
	};
} else {
	if(_curTarget isKindOf "Ship") then {
		...
		if(alive _curTarget && {_curTarget isKindOf "Ship"} && {local _curTarget} && {count crew _curTarget isEqualTo 0}) then { _Btn2 ctrlEnable true;} else {_Btn2 ctrlEnable false};
	} else {
		if((typeOf (_curTarget) in _dlcVehicles) && !(288520 in getDLCs 1)) then {
			if(_curTarget isKindOf "Air") then {
			...
			if(alive _curTarget && {count crew _curTarget isEqualTo 0} && {canMove _curTarget}) then { _Btn2 ctrlEnable false;} else {_Btn2 ctrlEnable true;};
		};
	};
