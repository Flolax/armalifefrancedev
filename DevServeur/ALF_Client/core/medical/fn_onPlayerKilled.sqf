...
_unit spawn {
	...
	waitUntil {sleep 0.3; _Timer ctrlSetText format[localize "STR_Medic_Respawn",[(_maxTime - time),"MM:SS"] call BIS_fnc_secondsToString];
...
_unit spawn {
    ...
    _requestTime = time + 5;
    waitUntil {sleep 0.3; round(_requestTime - time) <= 0 || isNull _this};
...
[_unit] spawn {
	...
	waitUntil {sleep 0.3; if(speed _unit isEqualTo 0) exitWith {true}; life_deathCamera camSetTarget _unit; life_deathCamera camSetRelPos [0,3.5,4.5]; life_deathCamera camCommit 0;};
};
